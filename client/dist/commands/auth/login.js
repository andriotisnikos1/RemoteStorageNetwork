...
async function action() {
    try {
        const networkHost = await text({
            ...
            validate: (input) => {
                if (!input || input.includes("://") || input.includes("/"))
                    return "Please provide a valid network host";
                return;
            }
        });
        const pingSpinner = spinner();
        pingSpinner.start(`Pinging ${String(networkHost)}...`);
        const { data: { authAcceptable } } = await axios.get(`http://${String(networkHost)}/auth/ping`);
        if (!authAcceptable) {
        ...
        const { data: { token } } = await axios.get(`http://${String(networkHost)}/auth/login`, {
            headers: {
